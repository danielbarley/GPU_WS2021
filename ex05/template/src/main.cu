...
// matMul_Kernel
//
__global__ void matMul_Kernel(int matrixSize, float* matrixA, float* matrixB,
			      float* matrixC) {
	...
	if (elementIdx < matrixSize && elementIdy < matrixSize) {
		...
		for (int i = 0; i < matrixSize; ++i) {
			temp_sum += matrixA[elementIdy * matrixSize + i] *
				    matrixB[i * matrixSize + elementIdx];
		}
		matrixC[elementId] = temp_sum;
	}
}
...
//
__global__ void shMatMul_Kernel(int matrixSize, float* matrixA, float* matrixB,
				float* matrixC) {
	extern __shared__ float sh_Mem[];
	int tilewidth = blockDim.x;
	float* sh_MatrixA = &(sh_Mem[0]);
	float* sh_MatrixB = &(sh_Mem[tilewidth * tilewidth]);
	//float* sh_MatrixC = &(sh_Mem[2 * tilewidth * tilewidth]); Why allocation for C?

	int elementIdx = blockIdx.x * blockDim.x + threadIdx.x; // Col
	int elementIdy = blockIdx.y * blockDim.y + threadIdx.y; // Row
	int elementSh = threadIdx.y * tilewidth + threadIdx.x;
	...
	if (elementIdx < matrixSize && elementIdy < matrixSize) {
		...
		for (int tile = 0; tile < gridDim.x; ++tile) {
			// colalborative load
			sh_MatrixA[elementSh] = matrixA[elementIdy * matrixSize + (tile * tilewidth + threadIdx.x)];
			sh_MatrixB[elementSh] = matrixB[elementIdx + (tile * tilewidth + threadIdx.y) * matrixSize];

			// barrier to prevent data hazards
			__syncthreads();
...
int main(int argc, char* argv[]) {
	...
	if (!pinnedMemory) {
		pinnedMemory =
		    chCommandLineGetBool("pinned-memory", argc, argv);
	}
	...
	if (!pinnedMemory) {
		...
		h_matrixC = static_cast<float*>(
		    calloc(static_cast<size_t>(matrixSize), sizeof *h_matrixC));
	...
	cudaMalloc(&d_matrixA,
		   static_cast<size_t>(matrixSize * sizeof(*d_matrixA)));
	cudaMalloc(&d_matrixB,
		   static_cast<size_t>(matrixSize * sizeof(*d_matrixB)));
	cudaMalloc(&d_matrixC,
		   static_cast<size_t>(matrixSize * sizeof(*d_matrixC)));
	...
	if (h_matrixA == NULL || h_matrixB == NULL || h_matrixC == NULL ||
		...
		std::cout << "\033[31m***" << std::endl
			  << "*** Error - Allocation of Memory failed!!!"
			  << std::endl
			  << "***\033[0m" << std::endl;
		exit(-1);
	}
	...
	gridSize = ceil(static_cast<float>(matrixWidth) /
	...
	std::cout << "***" << std::endl
		  << "*** Grid Dim:  " << grid_dim.x << "x" << grid_dim.y << "x"
		  << grid_dim.z << std::endl
		  << "*** Block Dim: " << block_dim.x << "x" << block_dim.y
		  << "x" << block_dim.z << std::endl
		  << "***" << std::endl;
	...
	if (chCommandLineGetBool("shared", argc, argv)) {
		matMul_Kernel<<<grid_dim, block_dim>>>(matrixWidth, d_matrixA,
						       d_matrixB, d_matrixC);
	} else {
		shMatMul_Kernel<<<grid_dim, block_dim, sharedMemSize>>>(
		    matrixWidth, d_matrixA, d_matrixB, d_matrixC);
	}
	...
	if (!dontCheckResult) {
		float* h_matrixD = static_cast<float*>(calloc(
		    static_cast<size_t>(matrixSize), sizeof(*h_matrixD)));
		...
		if (!resultOk) {
			std::cout
			    << "\033[31m***" << std::endl
			    << "*** Error - The two matrices are different!!!"
			    << std::endl
			    << "***\033[0m" << std::endl;
	...
	std::cout << "***" << std::endl
		  ...
		  << 1e3 * memCpyH2DTimer.getTime() << " ms" << std::endl
		  << "***    Copy Bandwidth: "
		  << 1e-9 * memCpyH2DTimer.getBandwidth(2 * matrixSize *
							sizeof(*h_matrixA))
		  << " GB/s" << std::endl
		  << "***    Time to Copy from Device: "
		  << 1e3 * memCpyD2HTimer.getTime() << " ms" << std::endl
		  << "***    Copy Bandwidth: "
		  << 1e-9 * memCpyD2HTimer.getBandwidth(matrixSize *
							sizeof(*h_matrixA))
		  << " GB/s" << std::endl
		  << "***    Time for Matrix Multiplication: "
		  << 1e3 * kernelTimer.getTime() << " ms" << std::endl
		  << "***" << std::endl;
...
void printHelp(char* programName) {
	std::cout
	    ...
	    << "  Usage: " << std::endl
	    << "  " << programName
	    << " [-p] [-s <matrix_size>] [-t <threads_per_block>]" << std::endl
	    ...
	    << "" << std::endl
	    << "  -t <threads_per_block>|--threads-per-block "
	       "<threads_per_block>"
	    << std::endl
	    << "  The number of threads per block" << std::endl
	    << "" << std::endl
	    << "  -c|--no-checking" << std::endl
	    << "  Do not check the result of the matrix multiplication"
	    << std::endl
	    << "" << std::endl
	    << "  --print-matrix" << std::endl
	    << "  Print the output matrix (only recommended for small matrices)"
	    << std::endl
	    << std::endl;
}
